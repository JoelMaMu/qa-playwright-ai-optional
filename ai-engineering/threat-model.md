...
| **T**ampering | A PR edits an agent definition with an invisible-Unicode payload | `invisible-unicode-lint` in CI, mandatory review of `.github/` and `ai-engineering/prompts/` | **medium**: the lint detects concealment, not intent |
...
2. **No lexical filter solves this.** A pattern filter is always one rephrasing
   behind. `ai-engineering/policy/sanitize-snapshot.mjs` ships a test that **documents this
...
|---|---|---|
| Container network `internal: true`, no outbound route | **holds alone** | `qa-automation/sut/docker/compose.yaml` |
| **Client-side** tool allowlist | **holds alone** | `ai-engineering/policy/mcp-client-allowlist.json` |
| Fencing and redaction of page content | reduces likelihood | `ai-engineering/policy/sanitize-snapshot.mjs` |
...
`ai-engineering/policy/mcp-client-allowlist.json` is an allowlist rather than a denylist: a
