...
2. **No raw selector in a spec.** Selectors live in a page object under
   `tests/pages/`. If an element is missing, add the locator to the page object.
3. **No assertion in a page object.** Page objects expose locators and actions. A
   hidden assertion escapes the healer gate, which only analyses `tests/specs`.
4. **No hard-coded data.** It comes from `tests/data/seed.ts`.
